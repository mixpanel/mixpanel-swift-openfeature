...
VERSION="$1"
REPO="${2:-mixpanel/mixpanel-swift-openfeature}"

if [ -z "$VERSION" ]; then
  echo "Usage: $0 <version> [repo]"
  echo "Example: $0 1.1.0 mixpanel/mixpanel-swift-openfeature"
  exit 1
fi
...
TAG_PREFIX=$(jq -e -r '.openfeature.tag_prefix' "$MODULES_JSON" 2>/dev/null || echo "INVALID")
